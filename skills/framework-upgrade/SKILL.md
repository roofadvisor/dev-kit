...
1. `package.json` → `devDependencies["@roofadvisor/dev-kit"] = "github:roofadvisor/dev-kit#v<version>"`,
   the version this plugin reports in `installed_plugins.json` —
   `npm install -D "github:roofadvisor/dev-kit#v<version>"` writes both the entry and
   the lock; `npm ci` refuses a lock that does not yet know the package and is for CI
   and `verify`.
